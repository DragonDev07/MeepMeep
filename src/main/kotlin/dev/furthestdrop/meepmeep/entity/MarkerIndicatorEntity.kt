...
import dev.furthestdrop.meepmeep.toRadians
...
class MarkerIndicatorEntity(
    ...
    fun passed() {
        if (!passed) {
            passed = true
            animationController.anim(0.0, 200.0, dev.furthestdrop.meepmeep.anim.Ease.EASE_IN_OUT_CUBIC)
    ...
    fun reset() {
        if (passed) {
            passed = false
            animationController.anim(MARKER_CIRCLE_RADIUS, 200.0, dev.furthestdrop.meepmeep.anim.Ease.EASE_IN_CUBIC)
        }
    }
