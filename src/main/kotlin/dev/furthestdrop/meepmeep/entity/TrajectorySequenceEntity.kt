...
import dev.furthestdrop.meepmeep.toScreenCoord
...
class TrajectorySequenceEntity(
    ...
    private val trajectorySequence: dev.furthestdrop.meepmeep.trajectorysequence.TrajectorySequence,
    private var colorScheme: dev.furthestdrop.meepmeep.colorscheme.ColorScheme
) : ThemedEntity {
    companion object {
    ...
    private var lastSegment: dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TrajectorySegment? = null
    private var currentSegment: dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TrajectorySegment? = null
...
//        )
        ...
        for (i in 0 until trajectorySequence.size()) {
            when (val step = trajectorySequence.get(i)) {
                is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TrajectorySegment -> {
                ...
                is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TurnSegment -> {
                ...
                is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.WaitSegment -> {
                }
            }
        }
        ...
        for (i in 0 until trajectorySequence.size()) {
            ...
            if (segment is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.WaitSegment || segment is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TurnSegment) {
                segment.markers.forEach { marker ->
                    val pose = when (segment) {
                        ...
                        is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TurnSegment -> segment.startPose.copy(heading = segment.motionProfile[marker.time].x)
                        else -> Pose2d()
                    }
            ...
            } else if (segment is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TrajectorySegment) {
        ...
        if (trajectoryProgress == null) {
            ...
            for (i in 0 until trajectorySequence.size()) {
                ...
                if (currentTime + seg.duration > trajectoryProgress!!) {
                    if (seg is dev.furthestdrop.meepmeep.trajectorysequence.sequencesegment.TrajectorySegment) currentSegment = seg
